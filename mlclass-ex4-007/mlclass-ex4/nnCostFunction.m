...
J = (1 / m) * sum(sum(cost)) + (lambda / (2 * m)) * (sum(sum(Theta1ExcludingBias .^ 2)) + ...
...
Theta1ZeroBias = [zeros(size(Theta1, 1), 1) Theta1ExcludingBias];
Theta2ZeroBias = [zeros(size(Theta2, 1), 1) Theta2ExcludingBias];
